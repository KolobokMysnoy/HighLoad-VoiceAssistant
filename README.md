...
Алиса хранит диалоги 14 дней, после чего, если пользователь не запросил, удаляет их. Предположим, что Алиса отправляет аудио и принимает аудио от пользователей. Также пользователь может общаться с ней при помощи чата.
...
В докладе[^12] говорится, что Алиса постоянно передаёт речь пользователя для её анализе на сервере, поэтому из пикового RPS рассчитаем нагрузку на сервер. Предположим, что битрейт у Алисы и у Яндекс Музыки одинаковый и равный 192 Кбит/c[^11]. Тогда получается, что пропускная способность Алисы должна быть равной 192 * 2 546 = 477 Мбит/с.

Рассчитаем количество трафика, потребляемое для связи с пользователями за день, используя информацию с расчетов текстовых запросов. Среднее время в секунд на запрос, которые нужно озвучить = (29 + 10) * (60/80) = 30 сек, как на разговор пользователя, так и на ответ Алисы. Тогда вес одного запроса будет равен 30 * 192 / 8 = 720 КБ. Тогда за день получаем 720 * 110 млн. / (1024^2^) = 75 531 ГБ.
...
***Сетевой трафик по темам***

Предположим из статистики[^8][^9][^13], что MVP запросы в течении дня делятся в следующем процентом отношении. Общий трафик за день составляет 75 531 ГБ, тогда из процентного соотношения можно найти трафик для темы.

| Запрос | Процент | Потребляемый трафик ГБ |
|--|--|--|
| Поиск информации |  40% | 30 212 + 16* |
| Погода | 25% | 18 882 |
| Музыка | 10% | 7 553 |
| Фильмы | 5% | 3 776 |
| Навигация | 20% | 15 106 |
\* *Дополнительные запросы на поиск информации*

Для получения данных для пропускной способности для каждой темы, будем предполагать, что 100% это пиковый RPS.

| Запрос | Процент | Пропускная способность Кб/c |
|--|--|--|
| Поиск информации |  40% | 195 379 |
| Погода | 25% | 122 112 |
| Музыка | 10% | 48 844 |
| Фильмы | 5% | 24 422 |
| Навигация | 20% | 97 689 |
...
| Пиковое потребление | 477 Мбит/с |
