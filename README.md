...
Алиса хранит диалоги в базе данных в таблице Questions. Пользователем может быть либо устройство с Session, который выдаётся, либо с аккаунтом, где есть login и password. 

Для сбора статистики, чтобы понять, как улучшить систему используется таблица Statistics, которая будет обновляться раз в сутки ночью, так как не требует актуальных данных.
