...
Рассчитаем количество занимаемой памяти для аудио запросов. Используя информацию с расчетов текстовых запросов. Среднее время в секунд на запрос, которые нужно озвучить = (29 + 10) * (60/80) = 30 сек. Предположим, что битрейт у Алисы и Яндекс музыки один, тогда вес занимаемый одним запросом будет равен 30 * 192 / 8 = 720 КБ.
