...
Весь входящий трафик сначала попадает на "Big Balancer", который определяет минимально загруженный балансировщик и перенаправляет на него. Так как в Алисе поддерживается постоянное соединение, то мы можем пожертвовать временем на более долгое первичное подключение, но быструю работу потом.
...
## 6.1 Денормализация <a name="bd-denormilize"></a>

Проведём денормализацию данных. Для более удобной аналитики, даныне в Audios будем хранить вместе с question, answer полями. 

## 6.2 Индексы <a name="bd-index"></a>

Основные запросы от пользователей:
- найти историю прошлых запросов
- добавить отзыв на ответ
- залогиниться

Основные запросы от аналитиков: 
- найти все записи удовлетворяющие требованию(наличие каких то ключевых слов)

Из этих запросов добавим индексы на поля login, question.

## 6.3 Выбор СУБД <a name="bd-subd"></a>
![Alt text](image-10.png)
|Субд| Функционал | Производительность| Надежность| Масштабирование| Резервирование| Поддержка |
|-|-|-|-|-|-|-|
|ORC на основе Hadoop| Возможность складирования огромного количества данных. В основ для долгосрочного хранения файлов | Медленная | Хранение только больших файлов | HDFS Federation, позволяет хранить большие объёмы, избегая ограничения стандартного Hadoop |  | Поддержка пользователей |
| MongoDB| OLTP | Надежная, резервные копирования |  | Шардирование по стандарту |  | Поддержка пользователей |
| |  | |  |  |  |  |



## 6.4 Шардинг <a name="bd-shard"></a>



## 6.5 Резервное копирование <a name="bd-reserve"></a>
