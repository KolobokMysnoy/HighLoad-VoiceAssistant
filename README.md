...
| Kafka | Audios, Questions, AnswerQuality(AudioStore) | Хранит временные данные для записи раз в 5 минут в Hadoop. Получает от сервера записи голоса полностью вместе с расшифровкой[^31] | Раз в 5 минут | 3 819 RPS</br> 90MB/s | 252ГБ |
...
<!-- TODO подумать над хранением в Kafka данных за неделю -->

<!-- TODO  ниже создать таблицу надёжность и улучшения для всех бд снизу-->
### Hadoop

<!-- TODO name node, подумать, куда их деть -->

Будет для надежности включена настройка на Hadoop 3 Erasure Encoding, которая позволит вместо копий хранить "parity cells".

<!-- TODO Сколько блоков нужно для хранения Erasure Encoding (34:00) -->
...
Ограничивать доступ по объёму запроса и количеству запросов в день. Поддерживает шардирование

<!-- TODO репликация в ClickHouse -->

Позволяет создавать реплику только таблиц, не целиком БД. Создается Backup раз в день автоматически.

### Kafka

Высокая производительность. Горизонтальное маштабирование: создание нескольких брокеров

Для обеспечения сохранности данных будет использоваться фактор репликации 3. Также создание Kafka-broker с помощью Kafka Raft metadata[^43], чтобы выдерживать нагрузку

Для обеспечения сохранности данных, Kafka будет хранить данные неделю.

### Tarantool

На каждом ДЦ создаются свои реплики. Между всеми репликами ставится связь Master-Master[^40]. Таким образом данные о пользователях будут на всех ДЦ.

In-memory. Индекс на логин пользователя, на сессию пользователя. 

Будет использовано круговое шардирование, для возможности добавления новых шардов в будущем.

<!-- TODO возможно пояснить детальнее -->

### AIStore и S3

AIStore[^41] это object database, и в ней будут храниться подборки голосовых записей для обучения моделей.
<!-- TODO подробнее описать, почему имеенно AIStore. Просто по ссылке взять материалы -->
AIStore был выбран за возможность более плотного взаимодействия с обучением моделей ML.
...
<!-- 
TODO глянуть с собесов примеры, оттуда добавить сервисы сюда (52:30)
