...
3.2 [Балансировка](#global-balance) <br>
3.3 [Итоговое покрытие](#global-coverage) <br>
4. [Список использованной литературы](#usageList) <br>
...
В России проникновение интернета составляет 88.2%[^19]. Также процент проникновения Алисы равен 57 / 127 * 100% = 44%, тогда коэффициент для пользователей Алисы равен 0,388 или 39%

Рассчитаем среднее количество пользователей на каждый федеральный округ России[^21].

|Федеральный округ|Пользуется Алисой|
|--|--| 
| Центральный |  |
| Приволжский |  |
| Сибирский |  |
...
Изучив плотность населения по регионам в РФ[^17], прохождение магистральных кабелей[^20], а также население федеральных округов[^21] 
Рассмотрим плотность людей по регионам в РФ[^17]. Из данной статистики можно сказать, что большая часть населения России живёт в западной части страны. Используя карту магистрального расположения кабелей[^20], можем расположить наши дата центры в местах, где больше всего подключений.
['15291297.28', '10899633.86', '6325404.76', '6323979.76', '5269591.86', '4658467.88', '3878177.4', '3003468.32']
Центральный         
Приволжский         
Сибирский           
Южный               
...
Балансировка будет осуществляться при помощи DNS, чтобы понять регион, к которому относится пользователь. После чего отправляется BGP Anycast к ближайшему дата центру.

## 3.3 Итоговое покрытие <a name="global-coverage"></a>

В итоге получилось 2 дата центра и 1 арендованный. На карте показано распределение нагрузки на каждый дата центр.

Дата центр в Санкт-Петербурге и Москве будет обслуживать центральную часть, самую нагруженную. Арендованный восточную часть России.
