...
Для связи серверов внутри дата центра будет использоваться Server Mesh c Sidecar Proxy. Так как приложения будту запущены в контейнерах, то также будет использоваться eBPF[^24], чтобы уменьшить количество потребляемой мощности процессора. 
