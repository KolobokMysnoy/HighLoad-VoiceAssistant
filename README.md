...
<!-- TODO горизонтальное масшатабирование hadoop -->
...
|-|-|-|-|-|-|-|-|
| Hadoop | Audios, Questions, AnswerQuality(AudioStore) | Хранение голосовых сообщений пользователя. Расшифровки сообщений. Качество ответов. | Горизонтальное маштабирование путём добавления нового namespace раз в год. | Внутри данные хранятся в нескольких экземлярах | 2 запроса в 12 часлов, для загрузки данных в ClickHouse и Nvidia AIStore | 1 запрос раз в 5 минут из Kafka | 330ТБ |
| ClickHouse | AnswerQuality<br>И отдельно темы запросов(QuestionsTheme) | Загрузка данных для аналитиков | Ограничивать доступ по объёму запроса и количеству запросов в день. Поддерживает шардирование | Позволяет создавать реплику только таблиц, не целиком БД. Создается Backup раз в день автоматически. Можно создавать вручную | Чтение будет с ограничениями | Запись раз в 12 часов | 2ТБ за 3 месяца |
| Kafka | Audios, Questions, AnswerQuality(AudioStore) | Хранит временные данные для записи раз в 5 минут в Hadoop. Получает от сервера записи голоса полностью вместе с расшифровкой[^31] | Высокая производительность. . Горизонтальное маштабирование: создание нескольких брокеров | Создание BackUP на случай падения. Также создание Kafka-broker с помощью Zookeeper, чтобы выдерживать нагрузку | Раз в 5 минут | 3 819 | 252ГБ |
| Tarantool | Users(Users, Sessions) | Хранит данные о пользователях и их сессиях | In-memory. Индекс на логин пользователя, на сессию пользователя. | На каждом ДЦ создаются свои реплики. Между всеми репликами ставится связь Master-Master[^40]. Таким образом данные о пользователях будут на всех ДЦ. | 3 819 | 0.5 | 27ГБ индексы </br> 125ГБ данные |
...
| Компонент | Что обеспечивает надежность|
|-|-|
| БД  | Реплекации данных на нескольких серверах. Перезапрос данных, в случае неудачного обращения |
| Система  | Под большой нагрузкой постепенная деградация распознавания речи(уменьшение запущенных сценариев). Допускается долгий ответ. Включение и настройка Alert оповещений, при сбоях в системе, по собираемой статистике(см. 8.1) |
| Flint  | Записывает данные всегда только один раз, при помощи опроса Hadoop о записи(2-ух фазовый коммит)  |
| ДЦ | Наличие нескольких ДЦ по стране, в случае отказа одного, трафик перейдёт на другой. </br> Наличие 2 входных роутеров, а также дублирование всех критических компонентов в системе | 
